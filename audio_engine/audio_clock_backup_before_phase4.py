...
class AudioClock:
    ...
    def update_frame_count(self, frames: int) -> None:
        ...
        with self._lock:
            ...
            old_beat = self._state.current_beat
            ...
            if abs(self._state.current_beat - old_beat) > 0.1:  # Beat changed by more than 0.1
                logger.debug(f"AudioClock: Frame update: +{frames} frames, time: {old_time:.3f}s → {self._state.current_time:.3f}s, beat: {old_beat:.3f} → {self._state.current_beat:.3f}")
    
    def set_beat_timestamps(self, timestamps: List[float], bpm: float) -> None:
        """Set beat timing information"""
        with self._lock:
            logger.info(f"AudioClock: Setting {len(timestamps)} beat timestamps, BPM: {bpm}")
            if len(timestamps) > 0:
                logger.debug(f"AudioClock: First timestamp: {timestamps[0]:.3f}s, Last timestamp: {timestamps[-1]:.3f}s")
            ...
            logger.debug(f"AudioClock: Set {len(timestamps)} beat positions")
    ...
    def get_current_beat(self) -> float:
        ...
        with self._lock:
            if not self._state.is_running:
                logger.debug(f"AudioClock: Not running, returning 0.0")
                return 0.0
            
            # Debug logging
            if len(self._beat_timestamps) == 0:
                logger.debug(f"AudioClock: No beat timestamps, using BPM fallback. current_time={self._state.current_time:.3f}s, bpm={self._state.current_bpm}")
            else:
                logger.debug(f"AudioClock: Using beat timestamps. current_time={self._state.current_time:.3f}s, current_beat={self._state.current_beat:.3f}, timestamps_count={len(self._beat_timestamps)}")
    ...
    def get_time_to_next_beat(self) -> Optional[float]:
        ...
        with self._lock:
            if not self._state.is_running or len(self._beat_timestamps) == 0:
                return None
