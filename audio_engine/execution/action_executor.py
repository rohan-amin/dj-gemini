...
class CompositeActionExecutor(ActionExecutor):
    ...
    def register_executor(self, action_type: str, executor: ActionExecutor) -> None:
        ...
        Args:
            action_type: Type of action (e.g., 'activate_loop', 'play', 'stop')
...
class LoopActionExecutor(ActionExecutor):
    """
    Executor for loop-related actions.
    
    Handles loop activation, deactivation, and modification using
    ...
    def __init__(self, deck):
        ...
        Args:
            deck: Deck instance for loop operations
        """
        self._deck = deck
        self._supported_actions = {'activate_loop', 'deactivate_loop', 'modify_loop'}
    ...
    def execute_action(self, action_type: str, parameters: Dict[str, Any], 
        ...
        Args:
            action_type: Type of loop action
        ...
        """
        if action_type == 'activate_loop':
            return self._execute_loop_activation(parameters, execution_context)
        elif action_type == 'deactivate_loop':
            return self._execute_loop_deactivation(parameters, execution_context)
        elif action_type == 'modify_loop':
            return self._execute_loop_modification(parameters, execution_context)
        else:
            logger.error(f"Unsupported loop action type: {action_type}")
            return False
    
    def can_execute(self, action_type: str) -> bool:
        """Check if can execute loop actions."""
        return action_type in self._supported_actions
    
    def _execute_loop_activation(self, params: Dict[str, Any], context: Dict[str, Any]) -> bool:
        """
        Execute loop activation with frame accuracy.
        
        Args:
            params: Loop parameters (start_at_beat, length_beats, repetitions)
            context: Execution context (action_id, target_frame, beat_number)
            
        Returns:
            True if loop activation was successful
        """
        try:
            # Extract parameters
            ...
            if start_beat is None or length_beats is None:
                logger.error(f"Missing required loop parameters: start_at_beat={start_beat}, length_beats={length_beats}")
                return False
            
            if length_beats <= 0 or repetitions <= 0:
                logger.error(f"Invalid loop parameters: length_beats={length_beats}, repetitions={repetitions}")
                return False
            
            # Use frame-accurate activation
            start_frame = self._deck.beat_manager.get_frame_for_beat(start_beat)
            end_frame = self._deck.beat_manager.get_frame_for_beat(start_beat + length_beats)
            
            # Validate frame positions
            if start_frame >= self._deck.total_frames:
                logger.error(f"Loop start frame {start_frame} beyond track length {self._deck.total_frames}")
                return False
            
            if end_frame > self._deck.total_frames:
            ...
            if success:
                ...
                logger.error(f"Failed to activate loop {action_id}")
            
            return success
            
        except Exception as e:
            logger.error(f"Exception during loop activation {context.get('action_id', 'unknown')}: {e}")
            return False
    
    def _execute_loop_deactivation(self, params: Dict[str, Any], context: Dict[str, Any]) -> bool:
        """
        Execute loop deactivation.
        
        Args:
            params: Deactivation parameters (may include specific loop_id)
            context: Execution context
            
        Returns:
            True if loop deactivation was successful
        """
        try:
            # Optionally support deactivating specific loops by ID
            loop_id = params.get('loop_id')
            action_id = context.get('action_id', 'unknown_deactivation')
            ...
            logger.error(f"Exception during loop deactivation {context.get('action_id', 'unknown')}: {e}")
            return False
    
    def _execute_loop_modification(self, params: Dict[str, Any], context: Dict[str, Any]) -> bool:
        """
        Execute loop modification (future feature).
        ...
        """
        # Placeholder for future loop modification features
        # Could support changing repetitions, length, etc.
        logger.warning("Loop modification not yet implemented")
        return False
